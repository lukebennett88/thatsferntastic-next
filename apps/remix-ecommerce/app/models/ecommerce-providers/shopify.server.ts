...
export function createShopifyProvider({
  ...
  async function query(query: string, variables?: any, nocache?: boolean) {
    ...
    let maxAge = typeof maxAgeSeconds === "function" ? maxAgeSeconds(request.clone()) : maxAgeSeconds;

    return fetch(request).then((res) => res.json());
  }

  return {
    async getCartInfo(items) {
      ...
      for (let item of json.data.nodes) {
        ...
        subtotal = subtotal.plus(new Decimal(item.priceV2.amount).times(itemInput.quantity));
      ...
      let formattedSubTotal = formatPrice({
        ...
        currencyCode: json.data.nodes.find((n: any) => !!n?.priceV2?.currencyCode).priceV2.currencyCode,
      });

      return {
        formattedShipping: "Calculated at checkout",
        formattedSubTotal: formattedSubTotal,
        formattedTaxes: "Calculated at checkout",
    ...
    async getFeaturedProducts() {
      let json = await query(getAllProductsQuery, {
        first: 12,
      });

      let products = json.data.products.edges.map(
        ({ node: { id, handle, title, images, priceRange, variants } }: any): Product => ({
    ...
    async getPage(slug) {
      let json = await query(getPageQuery, { query: `handle:${slug}` });
    ...
    async getPages() {
      let json = await query(getAllPagesQuery);
    ...
    async getProduct(slug, selectedOptions) {
      let json = await query(getProductQuery, { slug });

      if (!json.data.productByHandle) {
        return undefined;
      }

      let { id, handle, title, description, descriptionHtml, images, priceRange, options, variants } =
        json.data.productByHandle;

      let optionNames = new Set(options.map((o: any) => o.name));
      let optionsMap = new Map(
        selectedOptions?.filter((option) => optionNames.has(option.name)).map((option) => [option.name, option.value]),
      );
      ...
      for (let { node } of variants.edges) {
        ...
        if (
          node.selectedOptions.every(
            (option: any) => optionsMap.has(option.name) && optionsMap.get(option.name) === option.value,
          )
        ) {
    ...
    async getProducts(category, sort, search, cursor, perPage = 30, nocache) {
      let q = "";
      if (search) {
      ...
      let productsInfo = category ? json.data.collections.edges[0]?.node.products : json.data.products;

      let { edges, pageInfo } = productsInfo;

      let nextPageCursor: string | undefined = undefined;
      let products =
        edges?.map(({ cursor, node: { id, handle, title, images, priceRange, variants } }: any): Product => {
...
function formatPrice({ amount, currencyCode }: { amount: string; currencyCode: string }) {
  return `${formatCurrency(Number(amount))} ${currencyCode}`;
}
