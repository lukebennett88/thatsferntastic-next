...
export let action: ActionFunction = async ({ request, params }) => {
  ...
  let redirectTo = validateRedirect(formData.get("redirect"), `/product/${params.slug}`);
...
export let loader: LoaderFunction = async ({ request, params }) => {
  ...
  let selectedOptions = Array.from(url.searchParams.entries()).map(([name, value]) => ({
    name,
    value,
  }));

  let session = await getSession(request);
  let product = await commerce.getProduct(slug, selectedOptions);
